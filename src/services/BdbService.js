...
export default class BdbService {
   ...
   */
  createKeypair = seed =>
    new this.orm.driver.Ed25519Keypair(bip39.mnemonicToSeed(seed).slice(0, 32));
  ...
  createTransaction = async (modelName, asset, metaData, fromKeypair) => {
    this.orm.define(modelName, asset);
    const result = await this.orm.models[modelName].create({
      keypair: fromKeypair,
      data: metaData
    });
    return result;
